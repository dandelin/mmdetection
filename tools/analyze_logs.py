...
def cal_train_time(log_dicts, args):
    for i, log_dict in enumerate(log_dicts):
        print(
            "{}Analyze train time of {}{}".format("-" * 5, args.json_logs[i], "-" * 5)
        )
        all_times = []
        ...
        print(
            "slowest epoch {}, average time is {:.4f}".format(
                slowest_epoch + 1, epoch_ave_time[slowest_epoch]
            )
        )
        print(
            "fastest epoch {}, average time is {:.4f}".format(
...
def plot_curve(log_dicts, args):
    ...
    for i, log_dict in enumerate(log_dicts):
        ...
        for j, metric in enumerate(metrics):
            ...
            assert (
                metric in log_dict[epochs[0]]
            ), "{} does not contain metric {}".format(args.json_logs[i], metric)
