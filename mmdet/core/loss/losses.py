...
def py_sigmoid_focal_loss(
    pred, target, weight, gamma=2.0, alpha=0.25, reduction="mean"
):
...
def weighted_sigmoid_focal_loss(
    ...
    return (
        torch.sum(
            sigmoid_focal_loss(pred, target, gamma, alpha, "none") * weight.view(-1, 1)
        )[None]
        / avg_factor
    )
...
def weighted_balanced_l1_loss(
    pred, target, weight, beta=1.0, alpha=0.5, gamma=1.5, avg_factor=None
):
    if avg_factor is None:
        avg_factor = torch.sum(weight > 0).float().item() / 4 + 1e-6
...
def weighted_iou_loss(
    pred, target, weight, style="naive", beta=0.2, eps=1e-3, avg_factor=None
):
