...
class RPN(BaseDetector, RPNTestMixin):
    ...
    def forward_train(self, img, img_meta, gt_bboxes=None, gt_bboxes_ignore=None):
        ...
        losses = self.rpn_head.loss(*rpn_loss_inputs, gt_bboxes_ignore=gt_bboxes_ignore)
        return losses
        ...
        if not rescale:
            for proposals, img_meta in zip(proposal_list, img_metas[0]):
                ...
                proposals[:, :4] = bbox_mapping(
                    proposals[:, :4], img_shape, scale_factor, flip
                )
        # TODO: remove this restriction
        return proposal_list[0].cpu().numpy()
